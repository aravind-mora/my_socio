...
/** POST /api/auth/demo-login — one-click company demo access (no OTP) */
export const apiDemoLogin = (email, password) =>
  client.post(ENDPOINTS.demoLogin, { email, password }).then((r) => r.data);
...
/** POST /api/chat { message, location? } → { success, reply } (auth required) */
export const apiChatbot = (message, location) =>
  client
    .post(ENDPOINTS.chatbot, { message, location }, { timeout: 30000 }) // 30s cap
    .then((r) => r.data);
