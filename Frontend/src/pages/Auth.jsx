...
import { apiSendOtp, apiVerifyOtp, apiSignup, apiLogin, apiVerifyLoginOtp, apiGoogleUrl, apiDemoLogin, setToken, errMsg } from "../api/client";
...
const ROLES = {
  customer: {
    backend: "CUSTOMER",
    emoji: "🧑‍💼",
    tag: "I want to book services",
    blurb: "Find trusted pros near you — plumbers, tutors, trainers, stylists & more. Request a service, pick a slot, pay safely.",
    features: [
      ["🔍", "Browse 1,200+ services in 15+ categories"],
      ["📅", "Pick your slot — pay after a provider is chosen"],
      ["💬", "Live chat channel with your provider"],
      ["⭐", "Rate & review after every service"],
    ],
    stat1: ["⭐ 4.8", "avg rating"],
    stat2: ["⚡ 24h", "response time"],
  },
  provider: {
    backend: "SERVICE_PROVIDER",
    emoji: "🚀",
    tag: "I want to offer services",
    blurb: "Turn your skills into income. Create your service, get requests, bid & chat with customers, grow your ratings.",
    features: [
      ["🧰", "Create your service with photos & slots"],
      ["💰", "Get bids accepted by real customers"],
      ["🤝", "Accept work and chat in a private channel"],
      ["📈", "Grow ratings and earn more"],
    ],
    stat1: ["💸 850+", "active providers"],
    stat2: ["🏘️ 40+", "cities covered"],
  },
};
...
export default function Auth() {
  ...
  /* ----- one-click demo login (company access, no OTP) ----- */
  const [demoBusy, setDemoBusy] = useState(false);

  const handleDemoLogin = async () => {
    setDemoBusy(true);
    try {
      const res = await apiDemoLogin("demouser17896@gmail.com", "demouser@14369");
      ...
      const msg = errMsg(err, "Demo login failed.");
      toast.error(msg + " Make sure the demo account exists.");
    } finally {
      setDemoBusy(false);
    }
  };
  ...
  const chooseScreen = (
    <div className="container page">
      {/* ---- hero header ---- */}
      <motion.div className="center" initial={{ opacity: 0, y: -14 }} animate={{ opacity: 1, y: 0 }}>
        <div className="row" style={{ justifyContent: "center", gap: 14 }}>
          <Logo size={62} />
          <div style={{ textAlign: "left" }}>
            <h1 style={{ fontSize: "2.1rem", lineHeight: 1 }}>Socio<span className="grad-text">Sphere</span></h1>
            <p className="muted" style={{ fontSize: "0.98rem", marginTop: 6 }}>Your neighbourhood, one sphere away 🪐</p>
          </div>
        </div>
        <div className="row wrap" style={{ justifyContent: "center", gap: 10, marginTop: 22 }}>
          {["✅ OTP-verified users", "🔒 Secure demo payments", "💬 Real-time chat channels", "🆓 Free to join"].map((t) => (
            <span key={t} className="glass" style={{ padding: "8px 16px", borderRadius: 999, fontSize: "0.85rem", color: "var(--text-dim)" }}>{t}</span>
          ))}
        </div>
        <p className="muted mt-20" style={{ fontSize: "1.05rem", maxWidth: 560, marginLeft: "auto", marginRight: "auto" }}>
          Join the sphere — book trusted local services <strong style={{ color: "var(--text)" }}>or</strong> turn your skills into income.
        </p>
      </motion.div>

      {/* ---- role panels ---- */}
      <div className="split mt-30">
            ...
            >
              <div>
                <div className="row" style={{ gap: 14, marginBottom: 14 }}>
                  <motion.div
                    style={{ width: 74, height: 74, borderRadius: 22, background: "var(--grad)", display: "grid", placeItems: "center", fontSize: "2.3rem", boxShadow: "0 12px 30px -10px rgba(255,122,89,0.7)" }}
                    animate={{ y: [0, -6, 0] }}
                  ...
                  <div style={{ alignSelf: "center" }}>
                    <h2 style={{ fontSize: "1.55rem", marginBottom: 4 }}>{r === "customer" ? "For Customers" : "For Providers"}</h2>
                    <span className="tag" style={{ background: "rgba(255,255,255,0.1)" }}>{R.tag}</span>
                  </div>
                </div>

                <p style={{ color: "var(--text-dim)", lineHeight: 1.6, fontSize: "0.95rem", marginBottom: 18 }}>{R.blurb}</p>

                <div style={{ display: "flex", flexDirection: "column", gap: 10, marginBottom: 18 }}>
                  {R.features.map(([icon, txt]) => (
                    <div key={txt} className="row" style={{ gap: 10, alignItems: "flex-start" }}>
                      <span style={{ width: 30, height: 30, borderRadius: 10, display: "grid", placeItems: "center", fontSize: "0.95rem", flexShrink: 0, background: "rgba(255,255,255,0.08)", border: "1px solid var(--stroke)" }}>{icon}</span>
                      <span style={{ fontSize: "0.92rem", lineHeight: 1.4, color: "var(--text)", paddingTop: 4 }}>{txt}</span>
                    </div>
                  ))}
                </div>
                ...
                <div className="row" style={{ gap: 10 }}>
                  <div className="glass" style={{ padding: "10px 14px", borderRadius: 14, flex: 1, minWidth: 110 }}>
                    <strong className="display" style={{ fontSize: "1.05rem" }}>{R.stat1[0]}</strong>
                    <div className="tiny">{R.stat1[1]}</div>
                  </div>
                  <div className="glass" style={{ padding: "10px 14px", borderRadius: 14, flex: 1, minWidth: 110 }}>
                    <strong className="display" style={{ fontSize: "1.05rem" }}>{R.stat2[0]}</strong>
                    <div className="tiny">{R.stat2[1]}</div>
                  </div>
                </div>
              </div>
      ...
      {/* ---- why sociosphere ---- */}
      <div className="grid-services mt-30" style={{ gridTemplateColumns: "repeat(auto-fit, minmax(220px, 1fr))" }}>
        {[
          ["🛡️", "Trusted & verified", "Email OTP on every account — no fakes."],
          ["⚡", "Fast to start", "Request a service in under a minute."],
          ["💬", "Live channels", "Chat instantly when a bid is accepted."],
          ["💸", "Fair payments", "Demo checkout + provider verification."],
        ].map(([icon, title, txt], i) => (
          <motion.div key={title} className="glass card-hover" style={{ padding: "20px 18px" }} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.3 + i * 0.07 }}>
            <div style={{ fontSize: "1.7rem", marginBottom: 8 }}>{icon}</div>
            <strong style={{ fontSize: "0.98rem" }}>{title}</strong>
            <p className="tiny" style={{ marginTop: 5, lineHeight: 1.5 }}>{txt}</p>
          </motion.div>
        ))}
      </div>
      ...
      <div className="center mt-20">
        <button
          ...
          disabled={demoBusy}
        >
          {demoBusy ? <Spinner /> : <><span style={{ fontSize: "1.2rem" }}>👁️</span> View Live Demo — no signup needed</>}
        </button>
        <p className="tiny mt-8" style={{ maxWidth: 380, margin: "8px auto 0", lineHeight: 1.5 }}>
          One-click access to a ready demo account. Explore services, requests, channels & the AI assistant — no OTP, no credentials to enter.
        </p>
      </div>
    </div>
  );
